...
async def obter_usuario_logado(request: Request) -> dict:
    try:
        ...
        token_cookie = request.cookies[NOME_COOKIE_AUTH]
        token_header = request.headers.get(NOME_HEADER_AUTH)
        if token_cookie.strip() == "" and token_header.strip() == "":
            return None
        token = token_cookie if token_cookie.strip() !="" else token_header.replace("Bearer","")
